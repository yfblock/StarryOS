...
use core::{any::Any, str, time::Duration};
...
fn parse_packet(raw: &[u8]) -> Result<Packet, CameraError> {
    ...
    info!("plen: {}, raw.len(): {}", plen, raw.len());
    if raw.len() != 4 + plen + 2 {
...
impl CviCamera {
    pub fn new() -> Self {
        use axconfig::{devices::PLIC_PADDR, plat::PHYS_VIRT_OFFSET};
        ax_println!("PLIC Address: {:#x}", PHYS_VIRT_OFFSET + PLIC_PADDR);
    ...
    fn ioctl(&self, cmd: u32, arg: usize) -> VfsResult<usize> {
        ...
        match cmd {
            CviCameraArgs::INIT => {
                ...
                self.inner.lock().ping().map_err(|_| LinuxError::EBADF)?; // Verify communication
            }
            CviCameraArgs::GetInfo => {
                ...
                let camera_info = self
                    ...
                    .get_camera_info()
                    .map_err(|_| LinuxError::EBADF)?;
                (arg as *mut CameraInfo).vm_write(camera_info)?;
            }
            CviCameraArgs::GetFrame => {
                // Handle GetFrame command
                let frame = self
                    .inner
                    .lock()
                    .get_frame()
